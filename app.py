...
if run_analysis:
    # 1. Prioritize secrets (Cloud), otherwise use the empty inputs from the sidebar
    gemini_key = st.secrets.get("GEMINI_API_KEY") or gemini_key_input
    tavily_key = st.secrets.get("TAVILY_API_KEY") or tavily_key_input

    # 2. Check if we have keys from either source
    if not gemini_key or not tavily_key:
        st.error("API Keys missing. Please ensure they are set in Streamlit Secrets or provided in the sidebar.")
        st.stop()

    # 3. Initialize clients with the retrieved keys
    gemini_client = get_gemini_client(gemini_key)
    tavily_client = TavilyClient(api_key=tavily_key)
    
    # ... keep the rest of your existing code exactly as it is (status, parsing, etc.)
